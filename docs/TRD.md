...
// 키 검증 결과(gemini.validateKey 반환형).
export interface KeyValidationResult { ok: boolean; error?: string; }
```
...
```ts
export const TEXT_MODEL = 'gemini-3.1-flash-lite';   // 텍스트(붙여넣기) 분석/키 검증 기본 모델
export const IMAGE_MODEL = 'gemma-4-31b-it';         // 캡처 이미지(비전) 전용 멀티모달 모델
export const GEMINI_MODEL = TEXT_MODEL;              // 하위 호환 별칭(기본=텍스트 모델)
...
// 키 동작 확인용 경량 호출. 현재 UI 흐름에서는 사용하지 않음(온보딩이 단순 저장으로 단순화됨).
...
- 키 검증(`validateKey`): 짧은 프롬프트("ping")로 1회 호출하는 헬퍼는 유지하되, 단순화된 온보딩 흐름에서는 현재 호출하지 않는다.
